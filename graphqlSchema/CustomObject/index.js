...
const typeDefs = `
  extend type Query {
    ...
    getCustomObjectsByModel(model: String!, locale: String, orderBy: CustomObjectOrderBy, filterJSONString: String, limit: Int, search: String): [CustomObject]
...
const resolvers = {
  Query: {
    getCustomObjectsByModel: (root, args, context, info) => {
      ...
      const filterObject = _.isEmpty(args.filterJSONString) ? {} : JSON5.parse(args.filterJSONString);
      ...
      const limit = args.limit === undefined ? undefined : parseInt(args.limit);
      ...
      const query = Object.assign(filterObject, { 'model': args.model, 'meta.deletedAt': { $exists: false } });
      // console.log('query', query);
      // console.log('limit', limit);
      
      return CustomObject.find(query, null, {
        sort, limit
      }).then(
        (records) => {
  ...
  Mutation: {
    addCustomObject: (root, args, context, info) => {
      ...
      return CustomObject.create({
          model: args.model,
          label: args.label,
          customOrder: !_.isNumber(args.customOrder) ? 0 : args.customOrder,
          meta: {
            createdAt: current,
            updatedAt: current
          },
